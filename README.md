...
# 运行 TUI 交互模式（默认单 Agent / 多域路由取决于 MANIFEST_DIR）
go run ./cmd/mady/ tui

# 运行 HTTP/SSE API 服务器
go run ./cmd/mady/ serve
...
| `API_KEY` | — | LLM API 密钥 |
| `MANIFEST_DIR` | `./manifests` | Agent Manifest 文件目录 |
| `WORKSPACE_DIR` | `./workspace` | Workspace 根目录 |
| `MADY_SINGLE_AGENT` | 未设置 | 设为 `1` 强制 TUI 单 Agent 模式 |
| `SESSION_DIR` | `./sessions` | 会话持久化目录 |
