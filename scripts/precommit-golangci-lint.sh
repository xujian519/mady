#!/usr/bin/env bash
# pre-commit hook: 对根模块（含 tools/ 目录包）、tui/、desktop/ 子模块分别运行 golangci-lint。
#
# 设计要点：
...
# 3. 未安装时阻断提交（fail-closed），并提示 `make install-lint` 与临时绕过方式。
...
if [ -x "$GOPATH_BIN" ]; then
    ...
    echo "[golangci-lint] 未安装，提交被阻断（fail-closed）。请运行 make install-lint。" >&2
    echo "[golangci-lint] 临时显式绕过：SKIP=golangci-lint git commit（记得尽快补装并注明理由）" >&2
    exit 1
fi
