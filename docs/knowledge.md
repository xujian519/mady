...
知识库检索 + 图谱增强 + SQLite 只读后端 + RRF 混合检索 + Agent 工具集成 + 法条索引构建。
...
`knowledge/loader/law_index.go` 的 `BuildLawArticleIndex` 从 wiki 拆分文件
（`~/.mady/knowledge/wiki/法律法规/法律/专利法-2020-拆分-*.md`）的 H3 标题
构建「条号 → 主题关键词」索引，供 CitationGate S2 知识源使用：

- 遍历拆分文件（排除目录/实施细则/part 文件）
- `lawcite.ParseChineseNumber` 解析条号
- 标题按「与/及/和/、」切分子短语（≥2 字去重）
- **v1 只索引专利法-2020**（82 条全覆盖）
- 输出：`ArticleIndex` 映射（int → []string 主题词）

## FileIndex 文件索引

`knowledge/fileindex/` 提供本地 MD 文件的递归扫描与缓存索引，
支持 `search_knowledge` 工具在 WIKI_PATH 或 KNOWLEDGE_DB_DIR 不可用时
作为兜底知识源。包含文件元数据缓存、增量扫描、文件名/内容关键词匹配。
