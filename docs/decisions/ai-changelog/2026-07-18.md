...
## docs: 代码质量现代化——slices.Contains/min 替换 + 全面审阅报告
...
## fix: Phase 7 剩余 High 修复（H2 H3 H6）+ 安全性审核（H4 H5 H7）
...
## fix: Phase 7 遗留 High 修复（H1 H11）+ 报告同步
...
## chore: M1 门禁加固（4/4）：pre-commit 跨机器兼容 + commit-msg 敏感路径门禁
...
## chore: M1 门禁加固（3/4）：清理无效 -short flag 与 forbidigo 死配置
...
## chore: M1 门禁加固（2/4）：codecov 配置合并并启用覆盖率门槛
...
## feat(integration): M1 门禁加固（1/4）：integration e2e 测试接入 CI
...
## fix(tui): TUI 质量审查修复落地（P1 主题检测 + P2 测试覆盖 + 支撑构造函数）
...
## fix: 规范合规全面修复（P0/P1/P2/P3 共 9 项）
...
## fix: Code Review 第二轮修复（5 项 P0/P1 + 1 项测试质量）
...
## refactor: P2-5 大文件拆分（4 个 >1000 行文件 → 20 个聚焦文件）
...
## 全量技术债务清理（P0 真实 Bug + P1 Lint 债务 + P2 结构性改进）
...
## feat: Open Design 思路引入 —— Tier 3 实现（最终阶段）
...
## feat: Open Design 思路引入 —— Tier 2 实现
...
## feat: 引入 Open Design 项目思路 —— 三阶段 Tier 1 实现
...
## feat(agentcore): ReasoningStrategyRouter 接入 4 个领域 Agent
...
## test: DoomLoop 集成测试——7 个端到端测试覆盖 5 个探测器和领域接线
...
## chore: 评估框架增强收尾——新度量注册 + `--format enhanced` CLI
...
## chore: 引用核验 P2a——CitationSource 知识源抽象 + S2 wiki 法条索引（82 条全覆盖）
...
## feat: 从 XiaoNuo Agent 引入评估框架增强、死循环检测和推理策略编排
...
## fix(tools): computer_use schema 畸形致 oMLX 500 + 引用核验端到端冒烟
...
## chore: 启用 computer_use 桌面控制工具（Assistant + Patent Agent）
...
## fix: 修复 PlanCompiler 边连接 bug——多假设子图被完全绕过
