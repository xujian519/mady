...
P2B 案例分布（2026-07-15 核对 `invalidation_decisions.json` 实际数据修正）：
- 结论：全部无效 34 件、部分无效 5 件、维持有效 1 件（⚠️ 此前文档误记为 16/14/10，实际严重偏向"全部无效"）
- 专利类型：发明 8 件、实用新型 16 件、外观设计 16 件
- 主要法条：专利法第22条第3款（创造性）为主，第22条第2款（新颖性）、第26条第3/4款（公开充分/支持清楚）、第46条第1款（程序决定）为辅

> **⚠️ P2B 已冻结（2026-07-15）**
>
> 经代码级核对，P2B 数据集存在两个根本性缺陷，已冻结，不再作为 live evaluation 基准：
>
> 1. **空壳输入**：40/40 条案例的「独立权利要求1」「主要证据」「请求理由」三个字段全部为空。Input 平均仅 94 字符（仅含专利号与类型），模型无法基于案件事实进行任何有效推理，只能凭专利号瞎猜结论。
> 2. **退化分布**：实际结论分布为 全部无效 34 / 部分无效 5 / 维持有效 1。模型对每道题都无脑回答"全部无效"即可命中 34/40 = 85% 的结论，数据集无法有效区分模型好坏。
>
> 因此本文档记录的 P2B 实时评估分数（修复后通过率 32.5%、`llm_judge` 均值 0.408）反映的是「DeepSeek 裸读空壳题目的猜测能力」，**不代表 Mady 产品能力，也不应作为优化 Prompt/模型的依据**。
>
> **冻结后的处置**：
> - `AllCases()` 仍含 P2B（保持静态 CI 门禁的结构完整性校验）
> - 新增 `ValidCases()`（排除 P2B）供 live evaluation 使用
> - `TestLiveDeepSeekInvalidationEval` 标记 `P2B FROZEN`，保留代码与缓存以备数据重建后复用
> - 重建方向：回到本地 2009 件原始 docx 重新提取完整字段（权利要求全文/对比文件/请求理由/决定要点）并平衡结论分布后再解冻
>
> 下一阶段以 **P2A（31 道真题，数据质量良好）为唯一有效 live evaluation 基准**，详见 [v0.7 报告](evaluation-baseline-v0.7.md)。
