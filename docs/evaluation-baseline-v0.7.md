...
**2c. L5 模拟 HITL 评估：LLM 模拟人工修订 ≠ 真实人工修订**

为验证「五步工作法是否应结合人机协作」，实施了 L5：Agent 产出初稿 → `mockHumanRevision`（LLM 模拟专家修订）→ 终稿评分。10 题实测 llm_judge 均值 **0.320**——五层最低。

但这**不意味着 HITL 有害**。逐题分析揭示了 mockHumanRevision 的两个相反效应：

| 初稿质量 | mockHumanRevision 效果 | 典型案例 | 真实专家行为 |
|----------|:---:|---------|------------|
| 高分初稿（L1≥0.80） | **有害**（−0.73/−0.80） | dec_065: 0.90→0.17, dec_098: 0.90→0.10 | 不会修改（初稿已正确） |
| 低分初稿（L1≤0.40） | **有益**（+0.53） | dec_067: 0.37→0.90 | 会修订（HITL 价值所在） |

**根因**：LLM 模拟的"专家修订"无法像真实专家一样判断「这个初稿已经够好了，不需要改」。它对**所有**初稿都做修改——包括已经正确的——从而引入新的不确定性。真实专家**不会把正确的结论改成错误的**。

**结论**：
1. **不能从 L5=0.320 得出"HITL 有害"**——这是 LLM 模拟修订的局限，非 HITL 本身的局限
2. **真实 HITL 的理论上限**应介于 L1（0.513）和完美修订（接近 1.0）之间——低分题被修复，高分题被保留
3. L5 揭示了**HITL 评估的方法论困境**：用 LLM 模拟人工修订无法准确估计真实 HITL 价值，需要真实专家盲测（P3）才能测量

**五层最终排序**：L1 通用 prompt（0.513）> L4 增强 prompt（0.410）> L0 裸 LLM（0.363）> L2 工具编排（0.334）≈ L5 模拟 HITL（0.320）
