...
| **P2B L2 +invalidation manifest** | 0%（0/10） | 0.407 | 0.267 | ⚠️ 低于 L1（−0.107）—— manifest 步骤设计需改进 |
...
**2. invalidation manifest 在真实案件上反而有害（L1→L2: −0.107）**

这与 P2A 考试题上的结论一致（manifest 对分析题有害）。根因相同：invalidation manifest 的 5 步流程（解析→逐项分析→证据链→独立论证→结论）产出的结构化报告偏离了 LLM judge 的评分标准。manifest 步骤需要改进——当前的步骤描述过于程序化，应更聚焦于分析深度而非流程完整性。
