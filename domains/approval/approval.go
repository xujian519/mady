...
type ApprovalGate struct {
	...
	deferredPersist DeferredPersistManager
...
// DeferredPersistManager handles commit/discard of guardrails-deferred
// messages when the human operator makes an approval decision.
// Implementations flush saved messages to persistent storage on CommitDeferred
// and drop them on DiscardDeferred.
type DeferredPersistManager interface {
	CommitDeferred()
	DiscardDeferred()
}

// WithDeferredPersist attaches a DeferredPersistManager so that
// RecordDecision triggers CommitDeferred or DiscardDeferred based on the verdict.
...
// (e.g. domains/patent.go, domains/legal.go).
type DeferredPersistFuncs struct {
	CommitFn  func()
	DiscardFn func()
}
...
func (g *ApprovalGate) RecordDecision(
	...
	case DecisionAdopted, DecisionModified:
		if g.deferredPersist != nil {
			g.deferredPersist.CommitDeferred()
		}
	case DecisionRejected:
		if g.deferredPersist != nil {
			g.deferredPersist.DiscardDeferred()
		}
	}
	return nil
}
