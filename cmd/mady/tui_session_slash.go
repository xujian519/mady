...
//
//nolint:dupl // novelty/invalidation 闭包结构相似但 GraphOption 类型不同（WithRetriever vs WithInvRetriever）
...
// 直接运行无效宣告分析 Pregel 图。
//
//nolint:dupl // 与 novelty 闭包结构相似但 GraphOption 类型不同（WithInvRetriever vs WithRetriever）
