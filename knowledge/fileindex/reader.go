...
// that would require Whisper or a speech-to-text service.
func (fr *FileReader) readAudio(path string, info os.FileInfo) *FileReadResult {
	ext := strings.ToLower(filepath.Ext(path))
	estimatedSeconds := int(info.Size() / (16 * 1024))
	if estimatedSeconds < 1 {
		estimatedSeconds = 1
	}
	estimatedMinutes := estimatedSeconds / 60
	asrTimeEstimate := estimatedMinutes / 2
...
// that would require Tesseract or an OCR service.
